...
static std::shared_ptr<PyDocument>
translate_gps_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataGpsTranslationSourceMode source_mode,
    MetadataGpsTranslationConflictPolicy conflict_policy, bool latitude_to_exif,
    bool longitude_to_exif, bool altitude_to_exif, uint32_t max_added_entries,
    ...
    uint64_t max_total_text_bytes)
{
    ...
    options.source_mode                 = source_mode;
    options.conflict_policy             = conflict_policy;
    options.latitude_to_exif            = latitude_to_exif;
    options.longitude_to_exif           = longitude_to_exif;
    options.altitude_to_exif            = altitude_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_gps_metadata(source->store, options,
                                            &translated);
    }
    if (result.status != MetadataGpsTranslationStatus::Ok) {
        std::string message = "metadata gps translation failed: ";
...
static std::shared_ptr<PyDocument>
translate_gps_destination_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataGpsTranslationSourceMode source_mode,
    MetadataGpsTranslationConflictPolicy conflict_policy, bool latitude_to_exif,
    bool longitude_to_exif, bool bearing_to_exif, bool distance_to_exif,
    uint32_t max_added_entries, uint32_t max_operations,
    uint32_t max_text_bytes_per_property, uint64_t max_total_text_bytes)
{
    ...
    options.latitude_to_exif            = latitude_to_exif;
    options.longitude_to_exif           = longitude_to_exif;
    options.bearing_to_exif             = bearing_to_exif;
    options.distance_to_exif            = distance_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    options.max_text_bytes_per_property = max_text_bytes_per_property;
    options.max_total_text_bytes        = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_gps_destination_metadata(source->store, options,
                                                        &translated);
    }
    if (result.status != MetadataGpsTranslationStatus::Ok) {
        std::string message = "metadata gps destination translation failed: ";
        message += metadata_gps_translation_status_name(result.status);
        if (result.failed_mapping != MetadataGpsTranslationMapping::None) {
            message += " for ";
            message += metadata_gps_translation_mapping_name(
...
static std::shared_ptr<PyDocument>
translate_gps_quality_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataGpsTranslationSourceMode source_mode,
    MetadataGpsTranslationConflictPolicy conflict_policy, bool status_to_exif,
    bool measure_mode_to_exif, bool dop_to_exif, bool differential_to_exif,
    bool horizontal_error_to_exif, uint32_t max_added_entries,
    uint32_t max_operations, uint32_t max_text_bytes_per_property,
    uint64_t max_total_text_bytes)
{
    ...
    options.source_mode                 = source_mode;
    options.conflict_policy             = conflict_policy;
    options.status_to_exif              = status_to_exif;
    options.measure_mode_to_exif        = measure_mode_to_exif;
    options.dop_to_exif                 = dop_to_exif;
    options.differential_to_exif        = differential_to_exif;
    options.horizontal_error_to_exif    = horizontal_error_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    options.max_text_bytes_per_property = max_text_bytes_per_property;
    options.max_total_text_bytes        = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_gps_quality_metadata(source->store, options,
                                                    &translated);
    }
    if (result.status != MetadataGpsTranslationStatus::Ok) {
        std::string message = "metadata gps quality translation failed: ";
        message += metadata_gps_translation_status_name(result.status);
        if (result.failed_mapping != MetadataGpsTranslationMapping::None) {
            message += " for ";
            message += metadata_gps_translation_mapping_name(
...
static std::shared_ptr<PyDocument>
translate_gps_text_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataGpsTranslationSourceMode source_mode,
    MetadataGpsTranslationConflictPolicy conflict_policy,
    bool satellites_to_exif, bool map_datum_to_exif,
    bool processing_method_to_exif, bool area_information_to_exif,
    uint32_t max_added_entries, uint32_t max_operations,
    uint32_t max_text_bytes_per_property, uint64_t max_total_text_bytes)
{
    ...
    options.source_mode                 = source_mode;
    options.conflict_policy             = conflict_policy;
    options.satellites_to_exif          = satellites_to_exif;
    options.map_datum_to_exif           = map_datum_to_exif;
    options.processing_method_to_exif   = processing_method_to_exif;
    options.area_information_to_exif    = area_information_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    options.max_text_bytes_per_property = max_text_bytes_per_property;
    options.max_total_text_bytes        = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_gps_text_metadata(source->store, options,
                                                 &translated);
    }
    if (result.status != MetadataGpsTranslationStatus::Ok) {
        std::string message = "metadata gps text translation failed: ";
        message += metadata_gps_translation_status_name(result.status);
        if (result.failed_mapping != MetadataGpsTranslationMapping::None) {
            message += " for ";
            message += metadata_gps_translation_mapping_name(
...
static std::shared_ptr<PyDocument>
translate_camera_text_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataTechnicalTranslationSourceMode source_mode,
    MetadataTechnicalTranslationConflictPolicy conflict_policy,
    bool spectral_sensitivity_to_exif, bool camera_owner_name_to_exif,
    bool body_serial_number_to_exif, bool lens_make_to_exif,
    bool lens_model_to_exif, bool lens_serial_number_to_exif,
    uint32_t max_added_entries, uint32_t max_operations,
    uint32_t max_text_bytes_per_property, uint64_t max_total_text_bytes)
{
    MetadataCameraTextTranslationOptions options;
    options.source_mode                  = source_mode;
    options.conflict_policy              = conflict_policy;
    options.spectral_sensitivity_to_exif = spectral_sensitivity_to_exif;
    options.camera_owner_name_to_exif    = camera_owner_name_to_exif;
    options.body_serial_number_to_exif   = body_serial_number_to_exif;
    options.lens_make_to_exif            = lens_make_to_exif;
    options.lens_model_to_exif           = lens_model_to_exif;
    options.lens_serial_number_to_exif   = lens_serial_number_to_exif;
    options.max_added_entries            = max_added_entries;
    options.max_operations               = max_operations;
    options.max_text_bytes_per_property  = max_text_bytes_per_property;
    options.max_total_text_bytes         = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_camera_text_metadata(source->store, options,
                                                    &translated);
    }
    if (result.status != MetadataTechnicalTranslationStatus::Ok) {
        std::string message = "metadata camera-text translation failed: ";
        message += metadata_technical_translation_status_name(result.status);
        if (result.failed_mapping
            != MetadataTechnicalTranslationMapping::None) {
            message += " for ";
...
static std::shared_ptr<PyDocument>
translate_apex_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataCaptureTranslationSourceMode source_mode,
    MetadataCaptureTranslationConflictPolicy conflict_policy,
    bool shutter_speed_value_to_exif, bool aperture_value_to_exif,
    bool brightness_value_to_exif, bool exposure_bias_value_to_exif,
    bool max_aperture_value_to_exif, uint32_t max_added_entries,
    uint32_t max_operations, uint32_t max_text_bytes_per_property,
    uint64_t max_total_text_bytes)
{
    MetadataApexTranslationOptions options;
    options.source_mode                 = source_mode;
    options.conflict_policy             = conflict_policy;
    options.shutter_speed_value_to_exif = shutter_speed_value_to_exif;
    options.aperture_value_to_exif      = aperture_value_to_exif;
    options.brightness_value_to_exif    = brightness_value_to_exif;
    options.exposure_bias_value_to_exif = exposure_bias_value_to_exif;
    options.max_aperture_value_to_exif  = max_aperture_value_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    options.max_text_bytes_per_property = max_text_bytes_per_property;
    options.max_total_text_bytes        = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_apex_metadata(source->store, options,
                                             &translated);
    }
    if (result.status != MetadataCaptureTranslationStatus::Ok) {
        std::string message = "metadata apex translation failed: ";
...
static std::shared_ptr<PyDocument>
translate_environment_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataCaptureTranslationSourceMode source_mode,
    MetadataCaptureTranslationConflictPolicy conflict_policy,
    bool temperature_to_exif, bool humidity_to_exif, bool pressure_to_exif,
    bool water_depth_to_exif, bool acceleration_to_exif,
    bool camera_elevation_angle_to_exif, uint32_t max_added_entries,
    uint32_t max_operations, uint32_t max_text_bytes_per_property,
    uint64_t max_total_text_bytes)
{
    ...
    options.source_mode                    = source_mode;
    options.conflict_policy                = conflict_policy;
    options.temperature_to_exif            = temperature_to_exif;
    options.humidity_to_exif               = humidity_to_exif;
    options.pressure_to_exif               = pressure_to_exif;
    options.water_depth_to_exif            = water_depth_to_exif;
    options.acceleration_to_exif           = acceleration_to_exif;
    options.camera_elevation_angle_to_exif = camera_elevation_angle_to_exif;
    options.max_added_entries              = max_added_entries;
    options.max_operations                 = max_operations;
    options.max_text_bytes_per_property    = max_text_bytes_per_property;
    options.max_total_text_bytes           = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_environment_metadata(source->store, options,
                                                    &translated);
    }
    if (result.status != MetadataCaptureTranslationStatus::Ok) {
        std::string message = "metadata environment translation failed: ";
        message += metadata_capture_translation_status_name(result.status);
        if (result.failed_mapping != MetadataCaptureTranslationMapping::None) {
            message += " for ";
            message += metadata_capture_translation_mapping_name(
...
translate_capture_additional_metadata_document(
    ...
    MetadataCaptureTranslationSourceMode source_mode,
    MetadataCaptureTranslationConflictPolicy conflict_policy,
    bool focal_length_in_35mm_film_to_exif, bool file_source_to_exif,
    bool scene_type_to_exif, uint32_t max_added_entries,
    uint32_t max_operations, uint32_t max_text_bytes_per_property,
    uint64_t max_total_text_bytes)
{
    ...
    options.focal_length_in_35mm_film_to_exif
        = focal_length_in_35mm_film_to_exif;
    options.file_source_to_exif         = file_source_to_exif;
    options.scene_type_to_exif          = scene_type_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    options.max_text_bytes_per_property = max_text_bytes_per_property;
    options.max_total_text_bytes        = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_capture_additional_metadata(source->store,
                                                           options,
                                                           &translated);
    }
    if (result.status != MetadataCaptureTranslationStatus::Ok) {
        std::string message = "metadata capture_additional translation failed: ";
        message += metadata_capture_translation_status_name(result.status);
        if (result.failed_mapping != MetadataCaptureTranslationMapping::None) {
            message += " for ";
            message += metadata_capture_translation_mapping_name(
...
static std::shared_ptr<PyDocument>
translate_capture_rational_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataCaptureTranslationSourceMode source_mode,
    MetadataCaptureTranslationConflictPolicy conflict_policy,
    bool subject_distance_to_exif, bool digital_zoom_ratio_to_exif,
    bool exposure_index_to_exif, bool flash_energy_to_exif,
    uint32_t max_added_entries, uint32_t max_operations,
    uint32_t max_text_bytes_per_property, uint64_t max_total_text_bytes)
{
    MetadataCaptureRationalTranslationOptions options;
    options.source_mode                 = source_mode;
    options.conflict_policy             = conflict_policy;
    options.subject_distance_to_exif    = subject_distance_to_exif;
    options.digital_zoom_ratio_to_exif  = digital_zoom_ratio_to_exif;
    options.exposure_index_to_exif      = exposure_index_to_exif;
    options.flash_energy_to_exif        = flash_energy_to_exif;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    options.max_text_bytes_per_property = max_text_bytes_per_property;
    options.max_total_text_bytes        = max_total_text_bytes;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_capture_rational_metadata(source->store, options,
                                                         &translated);
    }
    if (result.status != MetadataCaptureTranslationStatus::Ok) {
        std::string message = "metadata capture rational translation failed: ";
        message += metadata_capture_translation_status_name(result.status);
        if (result.failed_mapping != MetadataCaptureTranslationMapping::None) {
            message += " for ";
            message += metadata_capture_translation_mapping_name(
...
static std::shared_ptr<PyDocument>
translate_sensitivity_metadata_document(
    std::shared_ptr<PyDocument> source,
    MetadataCaptureTranslationSourceMode source_mode,
    MetadataCaptureTranslationConflictPolicy conflict_policy,
    uint32_t max_added_entries, uint32_t max_operations,
    uint32_t max_text_bytes_per_property, uint64_t max_total_text_bytes)
{
    MetadataSensitivityTranslationOptions options;
    options.source_mode                 = source_mode;
    options.conflict_policy             = conflict_policy;
    options.max_added_entries           = max_added_entries;
    options.max_operations              = max_operations;
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_sensitivity_metadata(source->store, options,
                                                    &translated);
    }
    if (result.status != MetadataCaptureTranslationStatus::Ok) {
        std::string message = "metadata sensitivity translation failed: ";
        message += metadata_capture_translation_status_name(result.status);
        if (result.failed_mapping != MetadataCaptureTranslationMapping::None) {
            message += " for ";
            message += metadata_capture_translation_mapping_name(
...
translate_editorial_metadata_document(
    ...
    bool transmission_reference_to_iptc,
    uint32_t max_source_properties, uint32_t max_added_entries,
    uint32_t max_operations, uint64_t max_total_text_bytes)
{
    ...
    {
        nb::gil_scoped_release gil_release;
        result = translate_xmp_editorial_metadata(source->store, options,
                                                  &translated);
    }
    if (result.status != MetadataDescriptiveTranslationStatus::Ok) {
        std::string message = "metadata editorial translation failed: ";
        message += metadata_descriptive_translation_status_name(result.status);
        if (result.failed_mapping
            != MetadataDescriptiveTranslationMapping::None) {
            message += " for ";
...
NB_MODULE(_openmeta, m)
{
    ...
    nb::enum_<MetadataTechnicalTranslationMapping>(
        ...
        .value("XmpBodySerialNumber",
               MetadataTechnicalTranslationMapping::XmpBodySerialNumber)
        .value("XmpLensMake", MetadataTechnicalTranslationMapping::XmpLensMake)
        .value("XmpLensModel",
               MetadataTechnicalTranslationMapping::XmpLensModel)
        .value("XmpLensSerialNumber",
    ...
    m.attr("METADATA_CAPTURE_ADDITIONAL_TRANSLATION_CONTRACT_VERSION")
        = nb::int_(kMetadataCaptureAdditionalTranslationContractVersion);
    m.attr("METADATA_CAPTURE_ADDITIONAL_TRANSLATION_MAX_ADDED_ENTRIES")
    ...
    m.attr("METADATA_ENVIRONMENT_TRANSLATION_MAX_TOTAL_TEXT_BYTES") = nb::int_(
        kMetadataEnvironmentTranslationMaxTotalTextBytes);
    m.attr("METADATA_APEX_TRANSLATION_CONTRACT_VERSION") = nb::int_(
        kMetadataApexTranslationContractVersion);
    m.attr("METADATA_APEX_TRANSLATION_MAX_ADDED_ENTRIES") = nb::int_(
    ...
    m.attr("METADATA_CAMERA_TEXT_TRANSLATION_MAX_TOTAL_TEXT_BYTES") = nb::int_(
    ...
    nb::enum_<MetadataGpsTranslationSourceMode>(
        m, "MetadataGpsTranslationSourceMode")
        .value("DirtyOnly", MetadataGpsTranslationSourceMode::DirtyOnly)
        .value("All", MetadataGpsTranslationSourceMode::All);

    nb::enum_<MetadataGpsTranslationConflictPolicy>(
        m, "MetadataGpsTranslationConflictPolicy")
        .value("PreserveExisting",
    ...
    nb::enum_<MetadataGpsTranslationMapping>(m, "MetadataGpsTranslationMapping")
        .value("None", MetadataGpsTranslationMapping::None)
        .value("ExifGpsLatitude",
               MetadataGpsTranslationMapping::ExifGpsLatitude)
        ...
        .value("ExifGpsDestLatitude",
               MetadataGpsTranslationMapping::ExifGpsDestLatitude)
        .value("ExifGpsDestLongitude",
               MetadataGpsTranslationMapping::ExifGpsDestLongitude)
        .value("ExifGpsDestBearing",
               MetadataGpsTranslationMapping::ExifGpsDestBearing)
        .value("ExifGpsDestDistance",
               MetadataGpsTranslationMapping::ExifGpsDestDistance)
        .value("ExifGpsStatus", MetadataGpsTranslationMapping::ExifGpsStatus)
        .value("ExifGpsMeasureMode",
               MetadataGpsTranslationMapping::ExifGpsMeasureMode)
        .value("ExifGpsDop", MetadataGpsTranslationMapping::ExifGpsDop)
        .value("ExifGpsDifferential",
               MetadataGpsTranslationMapping::ExifGpsDifferential)
        .value("ExifGpsHPositioningError",
               MetadataGpsTranslationMapping::ExifGpsHPositioningError)
        .value("ExifGpsSatellites",
               MetadataGpsTranslationMapping::ExifGpsSatellites)
        .value("ExifGpsMapDatum",
               MetadataGpsTranslationMapping::ExifGpsMapDatum)
        .value("ExifGpsProcessingMethod",
               MetadataGpsTranslationMapping::ExifGpsProcessingMethod)
        .value("ExifGpsAreaInformation",
               MetadataGpsTranslationMapping::ExifGpsAreaInformation);
    ...
    m.attr("METADATA_GPS_DESTINATION_TRANSLATION_CONTRACT_VERSION") = nb::int_(
        kMetadataGpsDestinationTranslationContractVersion);
    m.attr("METADATA_GPS_DESTINATION_TRANSLATION_MAX_ADDED_ENTRIES") = nb::int_(
        kMetadataGpsDestinationTranslationMaxAddedEntries);
    m.attr("METADATA_GPS_DESTINATION_TRANSLATION_MAX_TOTAL_TEXT_BYTES")
        = nb::int_(kMetadataGpsDestinationTranslationMaxTotalTextBytes);
    m.attr("METADATA_GPS_QUALITY_TRANSLATION_CONTRACT_VERSION") = nb::int_(
        kMetadataGpsQualityTranslationContractVersion);
    m.attr("METADATA_GPS_QUALITY_TRANSLATION_MAX_ADDED_ENTRIES") = nb::int_(
        kMetadataGpsQualityTranslationMaxAddedEntries);
    m.attr("METADATA_GPS_QUALITY_TRANSLATION_MAX_TOTAL_TEXT_BYTES") = nb::int_(
        kMetadataGpsQualityTranslationMaxTotalTextBytes);
    m.attr("METADATA_GPS_TEXT_TRANSLATION_CONTRACT_VERSION") = nb::int_(
        kMetadataGpsTextTranslationContractVersion);
    m.attr("METADATA_GPS_TEXT_TRANSLATION_MAX_ADDED_ENTRIES") = nb::int_(
        kMetadataGpsTextTranslationMaxAddedEntries);
    m.attr("METADATA_GPS_TEXT_TRANSLATION_MAX_TOTAL_TEXT_BYTES") = nb::int_(
        kMetadataGpsTextTranslationMaxTotalTextBytes);

    m.attr("METADATA_GPS_TEXT_TRANSLATION_MAX_TEXT_BYTES_PER_PROPERTY")
        = nb::int_(kMetadataGpsTextTranslationMaxTextBytesPerProperty);

    nb::enum_<MetadataGpsTranslationStatus>(m, "MetadataGpsTranslationStatus")
        .value("Ok", MetadataGpsTranslationStatus::Ok)
        .value("NullOutput", MetadataGpsTranslationStatus::NullOutput)
        .value("SourceNotFinalized",
               MetadataGpsTranslationStatus::SourceNotFinalized)
        .value("InvalidOptions", MetadataGpsTranslationStatus::InvalidOptions)
        .value("AmbiguousSource", MetadataGpsTranslationStatus::AmbiguousSource)
        .value("IncompleteSource",
               MetadataGpsTranslationStatus::IncompleteSource)
        .value("InvalidSourceValue",
               MetadataGpsTranslationStatus::InvalidSourceValue)
        .value("ValueOutOfRange", MetadataGpsTranslationStatus::ValueOutOfRange)
        .value("UnsupportedPrecision",
               MetadataGpsTranslationStatus::UnsupportedPrecision)
        .value("UnsupportedGpsVersion",
               MetadataGpsTranslationStatus::UnsupportedGpsVersion)
        .value("ValueTooLong", MetadataGpsTranslationStatus::ValueTooLong)
        .value("SourceLimitExceeded",
               MetadataGpsTranslationStatus::SourceLimitExceeded)
        .value("NativeConflict", MetadataGpsTranslationStatus::NativeConflict)
        .value("EntryLimitExceeded",
               MetadataGpsTranslationStatus::EntryLimitExceeded)
        .value("OperationLimitExceeded",
               MetadataGpsTranslationStatus::OperationLimitExceeded)
        .value("InternalError", MetadataGpsTranslationStatus::InternalError);

    m.attr("METADATA_GPS_TRANSLATION_CONTRACT_VERSION") = nb::int_(
        kMetadataGpsTranslationContractVersion);
    m.attr("METADATA_GPS_TRANSLATION_MAX_ADDED_ENTRIES") = nb::int_(
        kMetadataGpsTranslationMaxAddedEntries);
    m.attr("METADATA_GPS_TRANSLATION_MAX_OPERATIONS") = nb::int_(
        kMetadataGpsTranslationMaxOperations);
    m.attr("METADATA_GPS_TRANSLATION_MAX_TEXT_BYTES_PER_PROPERTY") = nb::int_(
        kMetadataGpsTranslationMaxTextBytesPerProperty);
    m.attr("METADATA_GPS_TRANSLATION_MAX_TOTAL_TEXT_BYTES") = nb::int_(
        kMetadataGpsTranslationMaxTotalTextBytes);
    m.def("metadata_gps_translation_status_name",
          &metadata_gps_translation_status_name, "status"_a);
    m.def("metadata_gps_translation_mapping_name",
          &metadata_gps_translation_mapping_name, "mapping"_a);
    ...
    m.attr("METADATA_SENSITIVITY_TRANSLATION_CONTRACT_VERSION") = nb::int_(
        kMetadataSensitivityTranslationContractVersion);
    m.attr("METADATA_SENSITIVITY_TRANSLATION_MAX_ADDED_ENTRIES") = nb::int_(
        kMetadataSensitivityTranslationMaxAddedEntries);
    m.attr("METADATA_SENSITIVITY_TRANSLATION_MAX_TOTAL_TEXT_BYTES") = nb::int_(
    ...
    m.attr("METADATA_CAPTURE_RATIONAL_TRANSLATION_MAX_ADDED_ENTRIES")
        = nb::int_(kMetadataCaptureRationalTranslationMaxAddedEntries);
    m.attr("METADATA_CAPTURE_RATIONAL_TRANSLATION_MAX_TOTAL_TEXT_BYTES")
        = nb::int_(kMetadataCaptureRationalTranslationMaxTotalTextBytes);
    ...
    nb::class_<PyDocument>(m, "Document")
        ...
        .def("translate_gps_metadata", &translate_gps_metadata_document,
             "source_mode"_a = MetadataGpsTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataGpsTranslationConflictPolicy::FailOnConflict,
             "latitude_to_exif"_a = true, "longitude_to_exif"_a = true,
             "altitude_to_exif"_a  = true,
             "max_added_entries"_a = kMetadataGpsTranslationMaxAddedEntries,
        ...
        .def("translate_gps_destination_metadata",
             &translate_gps_destination_metadata_document,
             "source_mode"_a = MetadataGpsTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataGpsTranslationConflictPolicy::FailOnConflict,
             "latitude_to_exif"_a = true, "longitude_to_exif"_a = true,
             "bearing_to_exif"_a = true, "distance_to_exif"_a = true,
             "max_added_entries"_a
             = kMetadataGpsDestinationTranslationMaxAddedEntries,
             "max_operations"_a = kMetadataGpsTranslationMaxOperations,
             "max_text_bytes_per_property"_a
        ...
        .def("translate_gps_quality_metadata",
             &translate_gps_quality_metadata_document,
             "source_mode"_a = MetadataGpsTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataGpsTranslationConflictPolicy::FailOnConflict,
             "status_to_exif"_a = true, "measure_mode_to_exif"_a = true,
             "dop_to_exif"_a = true, "differential_to_exif"_a = true,
             "horizontal_error_to_exif"_a = true,
             "max_added_entries"_a
             = kMetadataGpsQualityTranslationMaxAddedEntries,
             "max_operations"_a = kMetadataGpsTranslationMaxOperations,
             "max_text_bytes_per_property"_a
             ...
             "max_total_text_bytes"_a
             = kMetadataGpsQualityTranslationMaxTotalTextBytes)
        .def("translate_gps_text_metadata",
             &translate_gps_text_metadata_document,
             "source_mode"_a = MetadataGpsTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataGpsTranslationConflictPolicy::FailOnConflict,
             "satellites_to_exif"_a = true, "map_datum_to_exif"_a = true,
             "processing_method_to_exif"_a = true,
             "area_information_to_exif"_a  = true,
             "max_added_entries"_a = kMetadataGpsTextTranslationMaxAddedEntries,
             "max_operations"_a    = kMetadataGpsTranslationMaxOperations,
             "max_text_bytes_per_property"_a
        ...
        .def("translate_technical_metadata",
             ...
             "max_total_text_bytes"_a
             = kMetadataTechnicalTranslationMaxTotalTextBytes)
        .def("translate_camera_text_metadata",
             &translate_camera_text_metadata_document,
             "source_mode"_a = MetadataTechnicalTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataTechnicalTranslationConflictPolicy::FailOnConflict,
             "spectral_sensitivity_to_exif"_a = true,
             "camera_owner_name_to_exif"_a    = true,
             "body_serial_number_to_exif"_a   = true,
             "lens_make_to_exif"_a = true, "lens_model_to_exif"_a = true,
             "lens_serial_number_to_exif"_a = true,
             "max_added_entries"_a
             = kMetadataCameraTextTranslationMaxAddedEntries,
             "max_operations"_a = kMetadataTechnicalTranslationMaxOperations,
             "max_text_bytes_per_property"_a
             ...
             "max_total_text_bytes"_a
             = kMetadataCameraTextTranslationMaxTotalTextBytes)
        .def("translate_apex_metadata", &translate_apex_metadata_document,
             "source_mode"_a = MetadataCaptureTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataCaptureTranslationConflictPolicy::FailOnConflict,
             "shutter_speed_value_to_exif"_a = true,
             "aperture_value_to_exif"_a      = true,
             "brightness_value_to_exif"_a    = true,
             "exposure_bias_value_to_exif"_a = true,
             "max_aperture_value_to_exif"_a  = true,
             "max_added_entries"_a = kMetadataApexTranslationMaxAddedEntries,
        ...
        .def("translate_capture_additional_metadata",
             ...
             "conflict_policy"_a
             = MetadataCaptureTranslationConflictPolicy::FailOnConflict,
             "focal_length_in_35mm_film_to_exif"_a = true,
             "file_source_to_exif"_a = true, "scene_type_to_exif"_a = true,
             "max_added_entries"_a
        ...
        .def("translate_environment_metadata",
             &translate_environment_metadata_document,
             "source_mode"_a = MetadataCaptureTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataCaptureTranslationConflictPolicy::FailOnConflict,
             "temperature_to_exif"_a = true, "humidity_to_exif"_a = true,
             "pressure_to_exif"_a = true, "water_depth_to_exif"_a = true,
             "acceleration_to_exif"_a           = true,
             "camera_elevation_angle_to_exif"_a = true,
             "max_added_entries"_a
             = kMetadataEnvironmentTranslationMaxAddedEntries,
             "max_operations"_a = kMetadataCaptureTranslationMaxOperations,
             "max_text_bytes_per_property"_a
        ...
        .def("translate_capture_rational_metadata",
             &translate_capture_rational_metadata_document,
             "source_mode"_a = MetadataCaptureTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataCaptureTranslationConflictPolicy::FailOnConflict,
             "subject_distance_to_exif"_a   = true,
             "digital_zoom_ratio_to_exif"_a = true,
             "exposure_index_to_exif"_a = true, "flash_energy_to_exif"_a = true,
             "max_added_entries"_a
        ...
        .def("translate_light_source_metadata",
             ...
             "max_total_text_bytes"_a
             = kMetadataLightSourceTranslationMaxTotalTextBytes)
        .def("translate_sensitivity_metadata",
             &translate_sensitivity_metadata_document,
             "source_mode"_a = MetadataCaptureTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataCaptureTranslationConflictPolicy::FailOnConflict,
        ...
        .def("translate_location_to_structured_metadata",
             ...
             "max_total_text_bytes"_a
             = kMetadataDescriptiveTranslationMaxTotalTextBytes)
        .def("translate_editorial_metadata",
             &translate_editorial_metadata_document,
             "source_mode"_a
             = MetadataDescriptiveTranslationSourceMode::DirtyOnly,
             "conflict_policy"_a
             = MetadataDescriptiveTranslationConflictPolicy::FailOnConflict,
             "headline_to_iptc"_a = true, "instructions_to_iptc"_a = true,
