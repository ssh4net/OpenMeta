...
``GlobalParametersIFD`` (``0x0190``) and DNG ``ExtraCameraProfiles``
(``0xC6F5``): prepare omits the source pointer and reports it as an unsupported
EXIF entry. An existing target pointer and its target-owned child bytes remain
unchanged during TIFF/DNG rewrite. Fresh output therefore contains neither a
dangling source offset nor a partially reconstructed auxiliary directory.
...
boxes from the same managed family. This includes the EXIF UUID
...
existing ``idat``, with data reference index 0 or an explicitly parsed
self-contained version-0 ``dinf``/``dref`` ``url `` or ``urn `` entry.
...
``iref`` relation endpoints, version-0 ``grpl`` item-group members, and
``ipma`` associations. A strip operation has no replacement, and multiple old
