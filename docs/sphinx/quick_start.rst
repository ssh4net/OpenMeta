...
If the host owns the encoder, use stable ``prepare_transfer_handoff(...)`` for
decoded re-emission. The lower-level experimental
``prepare_metadata_for_target_snapshot(...)`` entry point remains available
when direct bundle access or experimental raw-carrier reuse is required.
...
``residual_metadata_paths``. Native RAF/X3F/CRW metadata is positional;
RAF/X3F embedded decoding follows declared preview, FujiIFD, and section-JPEG
ranges without reading image data. Undeclared source-wide searches and
selected recursive enrichment paths remain explicit residuals. Use
``collect_read_transfer_source_diagnostics(...)`` for machine-readable
failure, scratch, MakerNote, and residual details.
