...
.. list-table::
   ...
   * - Exact EXIF XMP source
     ...
     - Flag
   * - ``GPSTimeStamp``
     - TimeStamp (7) and DateStamp (29)
     - ``timestamp_to_exif``
   * - ``GPSSpeedRef`` and ``GPSSpeed``
     - SpeedRef (12) and Speed (13)
     - ``speed_to_exif``
   * - ``GPSTrackRef`` and ``GPSTrack``
     - TrackRef (14) and Track (15)
     - ``track_to_exif``
   * - ``GPSImgDirectionRef`` and ``GPSImgDirection``
     - ImgDirectionRef (16) and ImgDirection (17)
     - ``image_direction_to_exif``
...
codes follow the `Adobe EXIF namespace <https://developer.adobe.com/xmp/docs/xmp-namespaces/exif/>`_
and `CIPA EXIF/XMP mapping <https://cipa.jp/std/documents/e/DC-X010-2017.pdf>`_.
...
.. list-table:: Sensitivity fields
   :header-rows: 1

   * - exifEX property
     - Native tag/type
   * - PhotographicSensitivity
     - 0x8827 SHORT, 1..65535
   * - SensitivityType
     - 0x8830 SHORT, 0..7
   * - StandardOutputSensitivity
     - 0x8831 LONG, 1..4294967295
   * - RecommendedExposureIndex
     - 0x8832 LONG, 1..4294967295
   * - ISOSpeed
     - 0x8833 LONG, 1..4294967295
   * - ISOSpeedLatitudeyyy
     - 0x8834 LONG, 1..4294967295
   * - ISOSpeedLatitudezzz
     - 0x8835 LONG, 1..4294967295
...
APEX and focal-plane/subject arrays are covered by the batches below.
...
* ``exifEX:LensSpecification`` (legacy ``exif`` accepted) maps to ``0xA432`` as
  four unsigned RATIONALs: minimum/maximum focal length, then minimum F-number
  at each endpoint. Positive focal lengths must be ordered. Apertures are
  positive or exactly ``0/0`` for unknown. One typed URational array or four
  dense ``[1]`` through ``[4]`` scalar properties are accepted. Numeric text
  converts exactly to unsigned 32-bit fractions; floating scalars, units,
  incomplete/mixed shapes and duplicate aliases fail.
* ``exif:ImageUniqueID`` maps to ``0xA420`` as 32 ASCII hex characters plus the
  serializer's terminal NUL. Case and leading zeros are preserved. No trimming,
  case folding, UUID generation, inference or hyphen removal is performed.

Under ``DirtyOnly``, a dirty lens member selects clean companions too. A lone
root tombstone or four dirty indexed tombstones removes the lens field; partial
member deletion fails. Image ID uses its own eligible scalar source/tombstone.
Native equivalence requires the correct shape and value. Rational equivalence
retains unreduced native fractions; ID equivalence allows one native terminal
NUL. All validation, conflicts and limits precede changes, including aliased
input/output. Preparation may allocate; the host synchronizes shared objects.

Limits are 2 added entries, 1024 operations, 128 bytes per text member and 544
aggregate text bytes. Fixed four-element typed arrays do not consume text budget.
Portable output emits a canonical ``exifEX`` RDF Seq with exact fraction text,
including ``0/0``. Managed canonicalization recognizes its indexed shape.
Generic validation accepts the exact unsigned lens aperture ``0/0`` slots,
including validated authoring inputs; unrelated zero denominators remain errors.
Malformed native lens/ID values are skipped. Decoding preserves ImageUniqueID
boundary whitespace so malformed padded IDs remain detectable.

This is a structural Exif 2.3 / CIPA XMP translation contract. It does not check
UUID version, global uniqueness or capture-time identity retention policy and
...
across nine APIs at 0.5.3. The spatial batch below extends this coverage;
...
.. list-table:: Focal-plane and subject mappings
   :header-rows: 1
   ...
   * - ``SubjectArea``
     - ``0x9214``
     - SHORT array, count 2, 3 or 4
     - ``subject_area_to_exif``
   * - ``SubjectLocation``
     - ``0xa214``
     - SHORT array, count 2
     - ``subject_location_to_exif``
...
`CIPA DC-008-2012, camera information tags <https://www.cipa.jp/std/documents/e/DC-008-2012_E.pdf>`_;
...
The spatial milestone brought coverage to 46 targets across ten APIs; 0.5.8 extends it to 65 targets across fifteen APIs.
...
* OECF (8828): Columns, Rows, Names and signed rational Values.
* SpatialFrequencyResponse (A20C): Columns, Rows, Names and unsigned rational Values.
* CFAPattern (A302): Columns, Rows and Values with color codes 0 through 6.
* DeviceSettingDescription (A40B): Columns, Rows and ordered setting-string Values.
...
.. list-table:: EXIF text mappings
   ...
   * - exif:ExifVersion
     - 9000
     - Four ASCII digits in UNDEFINED count 4; no NUL.
   * - exif:FlashpixVersion
     - A000
     - ``0100`` in UNDEFINED count 4; no NUL.
   * - exif:UserComment
     - 9286
     - Character-set prefix plus text; XMP scalar or ``[@xml:lang=x-default]``.
   * - exifEX:ImageTitle
     - A436
     - ASCII or UTF-8 text; empty means unknown.
   * - exifEX:Photographer, ImageEditor
     - A437, A438
     - ASCII or UTF-8 text; require explicit native IFD0 Artist.
   * - exifEX:CameraFirmware, RAWDevelopingSoftware, ImageEditingSoftware, MetadataEditingSoftware
     - A439–A43C
     - ASCII or UTF-8 text; require explicit native IFD0 Software.
   * - exifEX:CameraOwnerName, LensMake, LensModel
     - A430, A433, A434
     - ASCII or UTF-8 text. These overlap the earlier camera-text API.
