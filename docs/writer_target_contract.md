...
| Opaque MakerNote payloads | vendor MakerNote blobs and private nested IFDs | Keep the original raw `ExifIFD:MakerNote` payload by default, or follow explicit MakerNote policy | Drop | Decoded safe facts can still be carried through standard EXIF/XMP fields; the opaque vendor blob is not copied for rendered outputs. Decoded-only vendor MakerNote sub-IFDs are not reconstructed into a new blob; prepare reports them as non-serializable when no raw MakerNote payload is available. |
...
field widths. When all retained self-contained item locations can be represented
as absolute extents, OpenMeta compacts the rebuilt `iloc` base-offset field
width to zero for simpler reader compatibility.
